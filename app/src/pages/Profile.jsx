export default function Profile({ dark, setDark }) {
  ...
  const menuItems = [
    ...
    { icon: 'M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z', label: 'Notiser', badge: 3 },
    { icon: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 17h-2v-2h2v2zm2.07-7.75l-.9.92C13.45 12.9 13 13.5 13 15h-2v-.5c0-1.1.45-2.1 1.17-2.83l1.24-1.26c.37-.36.59-.86.59-1.41 0-1.1-.9-2-2-2s-2 .9-2 2H8c0-2.21 1.79-4 4-4s4 1.79 4 4c0 .88-.36 1.68-.93 2.25z', label: 'Hjalp och support' },
  ]
  ...
  return (
    <div style={{ background: bg, minHeight: '100vh', maxWidth: '390px', margin: '0 auto', paddingBottom: '80px' }}>
      ...
      <div style={{ margin: '0 16px 12px', background: cardBg, border: '0.5px solid ' + borderColor, borderRadius: '16px', padding: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px' }}>
          ...
          <div>
            <p style={{ margin: '0 0 2px', fontSize: '17px', fontWeight: 500, color: text, fontFamily: 'Georgia, serif' }}>Tradgardsalskare</p>
            <p style={{ margin: 0, fontSize: '13px', color: muted }}>garden@example.com</p>
          </div>
        </div>

        <div style={{ borderTop: '0.5px solid ' + borderColor, paddingTop: '16px', display: 'flex', justifyContent: 'space-around' }}>
          {[['5', 'Plantor'], ['142', 'Dagar aktiv'], ['89%', 'Lyckade']].map(([val, lbl]) => (
      ...
      <div style={{ margin: '0 16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {menuItems.map((item, i) => (
          <div key={i} style={{ background: cardBg, border: '0.5px solid ' + borderColor, borderRadius: '14px', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer' }}>
            ...
            {item.badge && (
              <span style={{ background: '#4E6E44', color: '#F2F0E8', fontSize: '11px', borderRadius: '20px', padding: '2px 8px' }}>{item.badge}</span>
            )}
            <span style={{ color: muted, fontSize: '18px' }}>›</span>
          </div>
        ))}

        <div style={{ background: cardBg, border: '0.5px solid ' + borderColor, borderRadius: '14px', padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', cursor: 'pointer', marginTop: '4px' }}>
          <span style={{ fontSize: '15px', color: '#E24B4A' }}>Logga ut</span>
        </div>

        <p style={{ textAlign: 'center', fontSize: '12px', color: borderColor, marginTop: '8px' }}>Version 1.0.0</p>
      </div>
    </div>
  )
}
